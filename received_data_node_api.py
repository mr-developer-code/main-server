...
def encrypt_parameter():
    ...
    if agg_lock.acquire(blocking=False):
        ...
        try:
            ...
            required_fields = ['enc_param_node', 'enc_param_global','context', 'hash']
            ...
            ## hashing
            print("hashing")
            data_hash = hashlib.sha256(b"".join(node_weight_ser) +b"".join(global_weight_ser) + server_context_ser ).hexdigest()
...
def aggregated_parameter():
    if agg_lock.locked():
        agg_lock.release()
        return jsonify(data), 200
    return jsonify({'message':"First send parameter for aggregation"})
